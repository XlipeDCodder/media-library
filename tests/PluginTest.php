...
/**
 * Plugin validation tests for MediaLibrary.
 *
...
describe('Plugin Manifest', function () {
    ...
    it('has required fields', function () {
        ...
        expect($manifest['name'])->toBe('musicplayer/media-library');
        expect($manifest['namespace'])->toBe('MediaLibrary');
    });
...
describe('Native Code', function () {
    it('has Android Kotlin file', function () {
        $kotlinFile = $this->pluginPath . '/resources/android/MediaLibraryFunctions.kt';

        expect(file_exists($kotlinFile))->toBeTrue();

        $content = file_get_contents($kotlinFile);
        expect($content)->toContain('package com.musicplayer.plugins.media_library');
        expect($content)->toContain('object MediaLibraryFunctions');
        expect($content)->toContain('BridgeFunction');
    });

    it('has iOS Swift file', function () {
        $swiftFile = $this->pluginPath . '/resources/ios/MediaLibraryFunctions.swift';

        expect(file_exists($swiftFile))->toBeTrue();

        $content = file_get_contents($swiftFile);
        expect($content)->toContain('enum MediaLibraryFunctions');
        expect($content)->toContain('BridgeFunction');
    });
...
describe('PHP Classes', function () {
    it('has service provider', function () {
        $file = $this->pluginPath . '/src/MediaLibraryServiceProvider.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Musicplayer\MediaLibrary');
        expect($content)->toContain('class MediaLibraryServiceProvider');
    });

    it('has facade', function () {
        $file = $this->pluginPath . '/src/Facades/MediaLibrary.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Musicplayer\MediaLibrary\Facades');
        expect($content)->toContain('class MediaLibrary extends Facade');
    });

    it('has main implementation class', function () {
        $file = $this->pluginPath . '/src/MediaLibrary.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Musicplayer\MediaLibrary');
        expect($content)->toContain('class MediaLibrary');
    });
});
